<?php
namespace Vsb\Deal;
use \Vsb\Deal\Customer as Customer;
use \Vsb\Deal\WooRequiredObject as G24Object;
use \Vsb\Store\DBConnector as DBConnector;
...
class Deal extends G24Object{
    ...
    public function byId($id){
        ...
        try{
            ...
            $this->_loaded = true;
            Vsb::debug("Deal is : ". json_encode($this->_jdata));
            $this->getShop();
            $this->getOrder($id);
            $this->getCustomer();
            return true;
            ...
            Vsb::debug("Deal [#{$id}] not found.".$e->getMessage());
            return false;
        }
        return false;
    }
    ...
    public function getCustomer(){
        ...
        $this->customer->sync();
        Vsb::debug("Customer is : ". json_encode($this->customer->toArray()));
    ...
    public function getPaymentTypes(){
        ...
        $sql.= " order by pt.id";
        $payments=[];
        Vsb::debug("getPaymentTypes sql query:".$sql);
        try{$payments = $this->db->selectAll($sql);}
        catch(\Exception $e){
            Vsb::debug("getPaymentTypes exception : ". $e);
        }
        return $payments;
    }
    public function getDeliveryTypes(){
        ...
        $sql.= " order by dt.id";
        Vsb::debug("getDeliveryTypes sql query:".$sql);
        try{$deliveries = $this->db->selectAll($sql);}
        catch(\Exception $e){
            Vsb::debug("getDeliveryTypes exception : ". $e);
        }
        return $deliveries;
    }
    ...
    protected function getShop(){
        ...
        Vsb::debug("Shop is : ". json_encode($this->shop));
    }
    ...
    protected function getKeyAndSecret(){
        ...
        try{
            ...
            Vsb::debug("No key [#{$this->doain_id}] found.".$e->getMessage());
            return false;
        }
        return false;
    }
