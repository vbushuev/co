...
class Customer extends G24Object{
    ...
    protected function create(){
        try{
            ...
            Vsb::debug("Get_by_email exception. ".$e->getMessage());
            try{
                ...
                Vsb::debug("creating response. ");
                Vsb::debug($resp);
                $this->_jdata = array_merge($this->_jdata,json_decode(json_encode($resp->customer),true));
                $this->customer_id = $this->id;
            }
            catch(\Exception $e){
                Vsb::debug("creating exception. ".$e->getMessage());
                $this->customer_id = 0;
            }
        }//$this->update([]);
    }
    protected function getCustomer(){
        ...
        Vsb::debug("get customer sql [".$sql."]");
        $user = $this->db->select($sql);
        $this->_jdata = array_merge($this->_jdata,$user);
        Vsb::debug("Customer passport: ".$this->_jdata["passport"]);
